...
# Every mid-year population endpoint needed by the migration series, including
# 30 June of the year before the first migration year.
...
tryCatch({
  ...
  population <- DBI::dbGetQuery(con, "SELECT r.region, m.age, m.sex, m.time,
    SUM(m.value) AS popn
...
stopifnot(isTRUE(all.equal(population_totals, coarsened_population, tolerance = 1e-10)),
          ...
          !anyDuplicated(population[c("region", "age", "sex", "time")]),
          setequal(population$region, regions))
